...
pub fn replace_mermaid_charts(
    ...
    for (event, offset) in Parser::new_ext(&source, Options::all())
        .into_offset_iter() {
            match event {
                ...
                Event::Text(ref code) | Event::Code(ref code) => {
                    if state.is_mermaid_block {
                        ...
                        let desc: CowStr = format!("Chapter {}, Graphic {}", chapterno.as_str(), state.counter)
                        .into();
    ...
    pulldown_cmark_to_cmark::cmark(dbg!(events).into_iter(), &mut buf).map_err(Error::CommonMarkGlue)?;
    Ok(buf)
}
...
"#,
        ...
        assert_matches!(iter.next(), Some(Event::Text(s)) => { assert!(s.contains("1.2.3")) });
